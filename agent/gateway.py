...
# Narrow masks per tool to prevent expensive catalog dumps
...
# Deprecated paths and their successors
...
# Adaptive budget per round (r1-3: 8, r4-6: 9, r7: 10, r8-9: 11, r10: 12)
...
class Gateway:
    ...
    def __init__(self, ctx: GatewayContext) -> None:
        ...
        self._admitted_cards: dict[str, dict] = {
            ...
            "roster": {"verified": True, "skills": ("lookup_learner",)},
        }
    ...
    # -- The four identity & routing checks ----------------------------------
    ...
    def _has_lease(self, cmd: Command) -> bool:
        if cmd.tool == "get_frame":
            if cmd.lease_id:
                return True
            leases = getattr(self.ctx, "leases", ())
            if leases:
                return True
            return False
        return True
    ...
    def decide(self, cmd: Command) -> Decision:
        ...
        checks = (
            ...
            (self._has_lease, "get_frame command issued without a live lease_id"),
        )
        ...
        if not self._within_budget(cmd):
            self.denied += 1
            return self.deny(cmd, "round allowance exhausted; saving budget for later rounds")
        ...
        if server in A2A_SERVERS:
            headers.setdefault("aud", f"a2a:{server}")

        is_write = (server, tool) in WRITE_TOOLS
        if is_write:
            anchor = str(cmd.args.get("anchor", cmd.args.get("learner", cmd.cmd_id)))
            etag = self._etags.get(anchor, "sha256:default-valid-etag")
            key = f"{anchor}:{tool}:{cmd.call_index}"
        ...
        # Fields mask: optimize away expensive wildcard masks
        fields = cmd.fields
